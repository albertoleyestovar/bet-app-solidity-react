...
export function Home() {
    ...
    const [betInfoList, setBetInfoList] = useState({});
    ...
    const getBettingInformation = async (_roundId) => {
        ...
        // set bet info per bet value
        let bInfo = {};
        betValues.map((value) => {
            const numJoined = betArr.filter((b) => b._betValue == value).length;
            const totalBetAmount = betArr.filter((b) => b._betValue == value).reduce((accumulator, currentValue) => accumulator + parseInt(currentValue._betAmount), 0);
            bInfo = {
                ...bInfo,
                [value] : {
                    numJoined,
                    totalBetAmount: totalBetAmount / multi
                }
        };
        setBetInfoList(bInfo);
    });
                                        ...
                                        >
                                            ...
                                            {betValues.map((value) => (
                                                <MenuItem key={value} value={value}>
                                                    {value} {betInfoList[value] && <>- Joined: {betInfoList[value].numJoined}, Total Bet: {betInfoList[value].totalBetAmount}</>}
                                                </MenuItem>
                                            ))}
                                        </Select>
                                        <TextField
